...
This fork keeps Mesh LLM's normal behavior while adding optional features for a personal multi-machine setup:

- Tailscale discovery for tagged tag:mesh-llm peers.
- mesh-llm dashboard / mesh-llm tui as a native terminal interface separate from the web console.
- Reuse of an explicitly supplied invite token after restart via ~/.mesh-llm/invite.token.
- Windows per-user startup with mesh-llm setup --service.

For a machine that should reconnect after a reboot, join it once and install the startup task:

    mesh-llm --join <token>
    mesh-llm setup --service

The running node already retries membership after transient disconnects. The remembered token is application-level membership; revoked or expired tokens still need to be replaced.
